...
unsigned int num_joints = 0, num_virtual_joints=0, total_joints=0;
...
//Do IK to t from initial joint positions in joints (overwrites)
//needs: num_joints and g_ik_solver 
bool ik_tool(tf::Transform t, std::vector<double> &joints)
{
  ...
  KDL::JntArray q_init(total_joints), q(total_joints);
  for ( unsigned int i = 0; i < total_joints; i++)
	{
...
void target_cb(const geometry_msgs::PoseStamped &t_msg)
{
  ...
  j_ik.resize(total_joints);
  if (g_actual_js.position.size() >= num_joints)
  {
    //assign real joint values to j_ik
    for ( unsigned int i = 0; i < num_joints ; i++)
    {
      j_ik[i] = g_actual_js.position[i]; 
    }

    //virtual joints default to zero
    for (unsigned int i = num_joints; i < total_joints ; i++)
    {
        j_ik[i] = 0.0; 
    }

//    for ( unsigned int i = 0; i < total_joints ; i++)
//      ROS_INFO( "j_ik[%d]: %0.2f [%0.2f %0.2f]", i, j_ik[i]*180.0/M_PI, 
//                q_min.data[i]*180.0/M_PI, q_max.data[i]*180.0/M_PI );
  }
  ...
  std::vector<double> actual_js = j_ik;
  actual_js.resize(num_joints);

  g_js.header.stamp = ros::Time::now();
  g_js.position = actual_js;
...
//  }
  ...
  ROS_INFO("parsed chain successfully");
  ...
  //Augment the KDL chain for pointing with a fake prismatic and ball joint
  //KDL::Segment tip_segment = chain.getSegment(chain.getNrOfSegments()-1);
  KDL::Segment p1 = KDL::Segment(KDL::Joint(KDL::Joint::TransX),
                      KDL::Frame(KDL::Rotation::RPY(M_PI/2.0, 0.0, 0.0),
                                 KDL::Vector(0.0,0.0,0.0)));
  KDL::Segment p2 = KDL::Segment(KDL::Joint(KDL::Joint::RotX),
                      KDL::Frame(KDL::Vector(0.0,0.0,0.0)));
  KDL::Segment p3 = KDL::Segment(KDL::Joint(KDL::Joint::RotY),
                      KDL::Frame(KDL::Vector(0.0,0.0,0.0)));
  KDL::Segment p4 = KDL::Segment(KDL::Joint(KDL::Joint::RotZ),
                      KDL::Frame(KDL::Vector(0.0,0.0,0.0)));
  chain.addSegment(p1);
  chain.addSegment(p2);
  chain.addSegment(p3);
  ...
  num_virtual_joints = 4;
  total_joints = num_joints + num_virtual_joints;

  q_min.resize(total_joints);
  q_max.resize(total_joints);
  g_js.name.resize(total_joints);
  g_js.position.resize(total_joints);
  g_actual_js.name.resize(total_joints);
  g_actual_js.position.resize(total_joints);
...
//  ROS_INFO("root: %s", tree_root_name.c_str());
  ...
  unsigned int i = 0;
  //q_min and q_max are arranged with tip/tool at index [num_joints-1] 
  while(link && i < num_joints)
  {
      printf("i = %d, num_j=%d, total_j=%d", i, num_joints, total_joints);
    boost::shared_ptr<const urdf::Joint> joint = robot_model.getJoint(link->parent_joint->name);
  ...
  //for the virtual prismatic joint set max upper limit
  g_js.name[total_joints-1] = "virtual_joint1";
  q_min.data[total_joints-4] = 0.0;
  q_max.data[total_joints-4] = 10;
  g_js.name[total_joints-1] = "virtual_joint2";
  q_min.data[total_joints-3] = -M_PI;
  q_max.data[total_joints-3] = M_PI;
  g_js.name[total_joints-1] = "virtual_joint3";
  q_min.data[total_joints-2] = -M_PI;
  q_max.data[total_joints-2] = M_PI;
  g_js.name[total_joints-1] = "virtual_joint4";
  q_min.data[total_joints-1] = -M_PI;
  q_max.data[total_joints-1] = M_PI;

  //ik_solver_vel and fk_solver_chain are generated from KDL::chain chain
