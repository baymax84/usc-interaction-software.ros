...
void SerialPort::configureSearch(const std::string DeviceDescription, const int speed, const std::string SearchPrefix, const char *format,
                ...
                const int tout)
{
        printf("INFO: Search for %s in speed %d\n",DeviceDescription.c_str(),speed);
        itsDevName = "search";
                ...
                const int tout)
{
  itsDevName = dev;
        ...
        if (strlen(format) != 3) printf("FATAL ERROR: Incorrect format string: %s\n", format);
        ...
        switch(format[0]) {
                ...
                default: printf("FATAL ERROR: Invalid charbits: %c (should be 5..8)\n", format[0]);
        }
        ...
        switch(format[1]) {
                ...
                default: printf("FATAL ERROR: Invalid parity: %c (should be N,E,O)\n", format[1]);
        }

        switch(format[2]) {
                case '1': itsStopBits = 1; break;
                case '2': itsStopBits = 2; break;
                default: printf("FATAL ERROR: Invalid stopbits: %c (should be 1..2)\n", format[2]);
        }
}
...
void SerialPort::perror(void)
{
        switch(serialErrno)
        {
                ...
                case serialErrOpenFailed:
                        printf("ERROR: SerialPort::Open() Failed\n");
                        break;
                case serialErrSpeedInvalid:
                        printf("ERROR: SerialPort::setSpeed() Invalid Speed Param\n");
                        break;
                case serialErrFlowInvalid:
                        printf("ERROR: SerialPort::setFlow() Invalid Flow Param\n");
                        break;
                case serialErrParityInvalid:
                        printf("ERROR: SerialPort::setParity() Invalid parity Param\n");
                        break;
                case serialErrCharsizeInvalid:
                        printf("ERROR: SerialPort::setCharSize() Invalid Char Size\n");
                        break;
                case serialErrStopbitsInvalid:
                        printf("ERROR: SerialPort::setStopBits() Invalid Stop Bits\n");
                        break;
                case serialErrOptionInvalid:
                        printf("ERROR: SerialPort::read() invalid option\n");
                        break;
                case serialErrOutput:
                        printf("ERROR: SerialPort::read() err output\n");
                        break;
                case serialErrReadFailed:
                        printf("ERROR: SerialPort::read() Failed\n");
                        break;
                case serialErrReadTimedOut:
                        printf("ERROR: SerialPort::read() Timed Out\n");
                        break;
                case serialErrWriteFailed:
                        printf("ERROR: SerialPort::Write() Failed\n");
                        break;
                case serialErrFcntlFailed:
                        printf("ERROR: SerialPort::Fcntl() Failed\n");
                        break;
                case serialErrTcGetAttrFailed:
                        printf("ERROR: SerialPort::tcgetattr() Failed\n");
                        break;
                case serialErrTcSetAttrFailed:
                        printf("ERROR: SerialPort::tcsetattr() Failed\n");
                        break;
                default:
