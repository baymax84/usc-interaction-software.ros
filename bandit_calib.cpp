...
void calibrateJointCall(const int & Id, const double & Angle_Increment)
{
	calibrateJoint(Id, Angle_Increment, position_zero);
}
...
void write_file()
{
	...
	for ( int z_id = 0; z_id < 16; z_id++){
		if(z_id == 2 || z_id == 3 || z_id == 9 || z_id == 10){
			calibrateJointCall(z_id, 20);
			true_zero(z_id);
		}
		else {
			calibrateJointCall(z_id, 10);
			true_zero(z_id);
		}
