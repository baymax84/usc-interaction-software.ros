...
int main( int argc, char* argv[] )
{
  ...
  ros::Publisher joint_pub_ = nh.advertise<sensor_msgs::JointState>("output_joint_state",10);
    ...
    {
      right_shoulder_angle_pitch = atan2(right_shoulder_elbow.y(), right_shoulder_elbow.x());
    ...
    {
      left_shoulder_angle_pitch = atan2(left_shoulder_elbow.y(),left_shoulder_elbow.x()) ;
    ...
    //if (joint_position_left_shoulder.fConfidence >= 0.5)
    {
      left_shoulder_angle_yaw = atan2(left_elbow_hand.y(),left_elbow_hand.x());  // left_shoulder_elbow.x()
    }
