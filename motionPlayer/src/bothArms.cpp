...
{
  if(argc != 6) {
      printf("bothArms: wrong number of inputs.\nUsage:\n   bothArms [frametime] [start time] [delay] [left file] [right file].\n");
      exit(0);
  }

  double tstep = atof(argv[1]);
  double tstart = atof(argv[2]);
  double delay = atof(argv[3]);  // time to wait before starting motions.
  char * fnameL = argv[4];
  char * fnameR = argv[5];
  ...
  while (ros::ok())
  {
    ...
    l_arm.startTrajectory(getTrajectoryStance(L_ARM), delay);
    // r_arm.startTrajectory(getTrajectoryStance(R_ARM), delay);
    while (ros::ok() && !l_arm.getState().isDone() /* (!r_arm.getState().isDone*/)
        usleep(10000);

    printf("L-playMotion | R-playMotion\n");
    // l_arm.startTrajectory(playMotion(L_ARM,tstep,tstart,fnameL), delay);

    r_arm.startTrajectory(playMotion(R_ARM,tstep,tstart,fnameR), delay);
    while (ros::ok() && (/*(!l_arm.getState().isDone()) ||*/ (!r_arm.getState().isDone())))
      usleep(10000);
...
//        pch = strtok ((char*)line.c_str()," ");
        NUM = atoi((const char*)line.c_str());
        printf("there are %d frames, from string %s.\n", NUM, (const char*)line.c_str());
     }
     else {
  ...
  ROS_INFO("got past file i/o if statements.");
  ...
  arm = -arm;  //just because of the way I have it set up.

  ROS_INFO("Got past goal (I probably won't get called)");
  ...
  for (int i=0; i<NUM; i++) {
      ...
      if(inFile.is_open()) {
         if(getline(inFile, line)) {
             ...
             pch = strtok ((char*)line.c_str()," ");
//             pch = strtok(NULL, " ");
             goal.trajectory.points[waypoint].positions[0] = angles::from_degrees(atof(pch)*arm);
             pch = strtok(NULL, " ");
             goal.trajectory.points[waypoint].positions[1] = angles::from_degrees(atof(pch));
             pch = strtok(NULL, " ");
             goal.trajectory.points[waypoint].positions[2] = angles::from_degrees(atof(pch)*arm);
             pch = strtok(NULL, " ");
