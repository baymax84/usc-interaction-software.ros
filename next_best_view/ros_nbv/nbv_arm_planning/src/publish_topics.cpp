...
void PublishTopics::arrowMsg(std::string name, geometry_msgs::Pose pose)
{
  ...
  arrow_msg.pose.position = pose.position;
  arrow_msg.pose.orientation = pose.orientation;
...
void PublishTopics::PathMsg(std::string topic_name, geometry_msgs::PoseArray& poses)
{
  // draw the table top bounding box
  ...
  path_msg.ns = topic_name;
  path_msg.id = 0;
  path_msg.type = visualization_msgs::Marker::LINE_STRIP;
  path_msg.action = visualization_msgs::Marker::ADD;

  path_msg.scale.x = 0.005;

  path_msg.color.g = 1.0;
  path_msg.color.a = 1.0;

  geometry_msgs::Point p;
  for(unsigned int ii=0; ii<poses.poses.size(); ii++)
  {
    p.x = poses.poses[ii].position.x;
    p.y = poses.poses[ii].position.y;
    p.z = poses.poses[ii].position.z;
    path_msg.points.push_back(p);
  }

  newVisualizationMarker(path_msg);
}




void PublishTopics::sampledPosesMsg(geometry_msgs::PoseArray& poses)
{
  for(unsigned int i=0; i<poses.poses.size();i++)
  {
    std::string s = boost::lexical_cast<std::string>( (int)i );
    std::string topic_name = "sampled_pose_" + s;

    visualization_msgs::Marker arrow_msg;

    arrow_msg.header.frame_id = tf_frame_;
    arrow_msg.header.stamp = ros::Time::now();
    arrow_msg.ns = topic_name;
    arrow_msg.id = 0;
    arrow_msg.type = visualization_msgs::Marker::ARROW;
    arrow_msg.action = visualization_msgs::Marker::ADD;
    ...
    arrow_msg.pose.position = poses.poses[i].position;
    arrow_msg.pose.orientation = poses.poses[i].orientation;
